...
const NavLink: React.FC<{ href: string; icon: React.ReactNode; label: string; isActive: boolean }> = ({ href, icon, label, isActive }) => {
  return (
    <a
      href={href}
      className={`flex items-center px-4 py-3 my-1 text-sm font-medium rounded-2xl transition-all duration-200 transform hover:scale-105 ${
        isActive
          ? 'bg-primary text-dark shadow-lg shadow-primary-500/25'
          : 'text-dark-400 hover:bg-dark-200 hover:text-primary-300 hover:shadow-md'
      }`}
    >
      {icon}
...
const Sidebar: React.FC<SidebarProps> = ({ currentPage }) => {
  return (
    <aside className="hidden md:flex flex-col w-64 bg-dark-100 border-r border-primary-800 shadow-2xl">
      <div className="flex items-center justify-center h-20 border-b border-primary-800">
        <h1 className="text-2xl font-bold text-primary bg-gradient-to-r from-primary to-primary-300 bg-clip-text text-transparent">
          FinancialPlan
        </h1>
      </div>
